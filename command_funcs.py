...
async def uwu(msg: str ): # = many_years):
    """Translates a string to UwU speak."""
    try:
        if not isinstance(msg, str):
            raise TypeError(f"Message {msg} is not a valid string.")
        elif len(msg) == 0:
...
async def uhm(msg: str):
# async def uhm(msg: str):
    """Returns a string that is very unsure of itself."""
    try:
        if type(msg) != str:
            raise TypeError("msg {msg} is not a valid string.")
        elif len(msg) == 0:
            raise ValueError("Please enter a message to be translated.")
...
async def imdad(msg: str, prefix: tuple = ("i'm ", "i am ", "im "), maxlen: int = 20):
    '''Does what dads do best:
    Returns "Hi <message body>, I'm Dad." for messages under maxlen.'''
    try:
        if type(msg) != str:
            raise TypeError(f"msg {msg} is not a valid string.")
        elif len(msg) == 0:
            # raise ValueError("Please enter a message to be translated.")
            return
    except (TypeError, ValueError) as exc:
    ...
    if len(msg) > maxlen:
        ...
        return f"Hi {msg.rstrip(string.punctuation)}, I'm Dad."
    else:
        return


async def shuffle_words(msg: str):
    '''Returns a syntactically correct message with word order and punctuation
    randomized.'''
    chars = r'[!&,.:;?]+'
